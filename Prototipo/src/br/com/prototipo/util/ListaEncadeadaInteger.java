...
public class ListaEncadeadaInteger {
	...
	private NoInteger head, tail;
	private int size = 1;

	public ListaEncadeadaInteger() {
	}

	public void add(Integer element, int position) throws ErroException {
			...
			if (head == null)
				...
				default:
					if (position > size) {
						...
						NoInteger previous = head;
						for (int i = 1; i < position - 1; i++) {
							previous = previous.getNext();
						}
						NoInteger next = previous.getNext();
						no.setNext(next);
						previous.setNext(no);
						previous = next = null;
					}
		...
		if (!isEmpty()) {
			...
			default:
				if (position > size)
					...
					NoInteger next = previous.getNext().getNext();
	...
	public Integer show(int position) {
